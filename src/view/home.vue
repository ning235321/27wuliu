<template>
	<div class ="sideBar">
		<el-menu class="el-menu-vertical-demo menu" theme="dark">
			<el-menu-item index="1">
			    <router-link to="/home/hello">
			        <i class="fa icon" aria-hidden="true"></i>首页 
			    </router-link>
			</el-menu-item>
			
			<el-menu-item index="2">
				<router-link to="/home/project">
			    	<i class="fa icon" aria-hidden="true"></i>包裹
			    </router-link>
			</el-menu-item>
			<el-menu-item index="3" :class="[this.isadmin=='true' ? 'nohidden': 'hidden']">
				<router-link to="/home/members" >
			    	<i class="fa icon" aria-hidden="true"></i>管理
			    </router-link>
			</el-menu-item>
			<el-menu-item index="4">
				<router-link to="/home/personal">
			    	<i class="fa icon" aria-hidden="true"></i>个人信息
			    </router-link>
			</el-menu-item>
		</el-menu>
		<topBars class="topbar"></topBars>
		<Levelbar class="breadcrumb1"></Levelbar> 
		
		<router-view class="content"></router-view>	
	</div>
</template>
<style type="text/css" scoped>
	/*左边的导航条*/
	.sideBar .menu{
		width: 200px;
		height: 100%;
		position: fixed;
		top: 0;
		bottom: 0;
		left: 0;
	}
	.sideBar .menu a{
		color: #bfcbd9;
		text-decoration: none;
	}
	.sideBar a.router-link-active{
		color: #20a0ff;
		text-decoration: none;
	}
	/*导航条图标*/
	.sideBar .icon{
		padding-left: 15px;
		padding-right: 15px;
	}
	/*路由内容*/
	.sideBar .content{
		margin-left: 200px;
	}
	/*头顶的导航*/
	.sideBar .topbar{
		clear: both;
		padding: 0;
		margin-top: 0;
		margin-left: 200px;
	}
	.sideBar .breadcrumb1{
		clear: both;
		padding: 15px;
		margin-left: 200px;
		font-size: 18px;
	}

	.hidden{
		display: none;
	}
	.nohidden{
		display: inline-block;
		width: 100%;
	}
	
</style>
<script>
	import TopBars from '@/view/NavBar/topBars';
	import Levelbar from '@/view/NavBar/Levelbar';
	import store from './../store';
  	export default {
  		components:{
  			topBars:TopBars,
  			Levelbar,
  		},
		  data(){
			  return {
				  isadmin:store.state.data.isadmin
			  }
		  },
		mounted:function(){
			console.log(this.isadmin);
		}
  	}
</script>