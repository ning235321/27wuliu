...
import {userGoodsAdd, getGoodsdata, goodsDelete,getuserdatabyID,userGoodsDelete} from '../../network/newhttp'
...
// }
    ...
    mounted(){
      store.commit('getUserbyId',this.userdata._id)
    this.dialogLocation = false;
      ...
      getGoodsdata().then((res)=>{
        ...
        this.allgoodsdata = res
        for (let i = 0; i < res.length; i++) {
          // console.log("22222222222");
          for (let j = 0; j < this.userdata.goodslist.length; j++) {
          if(res[i].goodsnum == this.userdata.goodslist[j]){
...
// 添加货物
     handleSaveadd () {
       ...
       this.$confirm('确认提交吗？', '提示', {
         ...
         let flag = 1
        for (let i = 0; i < this.allgoodsdata.length; i++) {
        if(this.form.goodsnum == this.allgoodsdata[i].goodsnum){
          for (let j = 0; j < this.tableData.length; j++) {
            if(this.form.goodsnum == this.tableData[j].goodsnum){
              flag=0
            }
          }
          if(flag){
            this.tableData.push(this.allgoodsdata[i])
          let gdata = {goodslist:this.allgoodsdata[i].goodsnum}
          userGoodsAdd(this.userdata._id,gdata)
					store.commit('getUserbyId',this.userdata._id)
					// store.commit('addGoodslistData',this.allgoodsdata[i].goodsnum)
        ...
        return
            ...
            this.$message({
           message: "操作失败！列表已存在该货物",
           type: 'error'
        });
        return
          }
        }else{if(i==this.allgoodsdata.length-1){
          this.$message({
           message: "操作失败！未找到该单号货物",
           type: 'error'
        });
        }
        }
...
// 删除货物
     handleDelete (index) {
       console.log(this.tableData[index].goodsnum);
        let todeletedata = { goodsnum: this.tableData[index].goodsnum}
          userGoodsDelete(store.state.data._id,todeletedata).then(()=>{
